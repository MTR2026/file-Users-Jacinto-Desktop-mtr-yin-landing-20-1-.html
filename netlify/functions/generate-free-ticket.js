...
async function generatePDF(ticket, label, pvp, school, name, quantity) {
  return new Promise(async (resolve, reject) => {
    ...
    const instrucciones = [
      { icon: '📍', bold: 'Cómo llegar:', text: 'Mad Fight Stadium, Av. de los Gavilanes, San Sebastián de los Reyes. Metro L10 estación Baunatal. Bus 151/153 desde Plaza de Castilla.' },
      ...(label.includes('VIP') ? [{ icon: '⭐', bold: 'Acceso VIP:', text: 'Entrada exclusiva por acceso VIP con acceso preferente. Incluye cena.' }] : []),
      ...(label.includes('Primera') ? [{ icon: '💺', bold: 'Primera Fila:', text: 'Asientos numerados asignados el día del evento en taquilla.' }] : []),
      { icon: '🚪', bold: 'Puertas:', text: 'Se abren 30 minutos antes del evento. El evento es PUNTUAL — no llegues tarde.' },
      { icon: '🚫', bold: 'Sin devoluciones:', text: 'Las entradas no son reembolsables bajo ningún concepto.' },
      { icon: '📱', bold: 'Acceso:', text: 'Presenta este QR en la entrada. Uso único y no transferible.' },
    ];

    let lineY = infoY + 10;
    instrucciones.forEach(item => {
      doc.fontSize(8.5).fillColor('#f0ede8').font('Helvetica-Bold')
        .text(`${item.icon} ${item.bold} `, pad, lineY, { continued: true, width: W - pad*2 })
